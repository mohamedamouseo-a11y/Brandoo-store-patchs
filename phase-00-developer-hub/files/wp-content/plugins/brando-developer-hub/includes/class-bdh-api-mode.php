...
final class BDH_API_Mode {
    ...
    private static function manifest(): array {
        ...
        foreach ($it as $entry) {
            ...
            if (!is_string($content)) { throw new RuntimeException('Failed to read managed file: ' . $relative); }
            $total += strlen($content);
            if ($total > self::MAX_BYTES) { throw new RuntimeException('API-mode baseline exceeds the 8 MB safety limit.'); }
            $map[$relative] = ['sha256'=>hash('sha256', $content), 'content'=>$content, 'size'=>strlen($content)];
            if (count($map) > self::MAX_FILES) { throw new RuntimeException('API-mode baseline contains too many managed files.'); }
        }
        ksort($map, SORT_STRING);
        foreach ($map as $path=>$meta) { $preview[] = ['direction'=>'local','status'=>'A','path'=>$path]; if (count($preview)>=500) break; }
        $fingerprintParts=[];
        foreach ($map as $path=>$meta) { $fingerprintParts[]=$path.':'.$meta['size'].':'.$meta['sha256']; }
        return ['map'=>$map,'hash'=>hash('sha256',implode("\n",$fingerprintParts)),'fileCount'=>count($map),'sizeBytes'=>$total,'files'=>$preview,'truncated'=>count($map)>500];
    }
    ...
    public static function init_preview(string $repo, string $branch): array {
        $repo=self::validate_repo($repo); $branch=self::validate_branch($branch);
        $info=BDH_Core::github_request('https://api.github.com/repos/'.$repo);
        $permissions=is_array($info['permissions']??null)?$info['permissions']:[];
        if (empty($permissions['push']) && empty($permissions['admin']) && empty($permissions['maintain'])) { throw new RuntimeException('GitHub token does not have write access to the selected repository.'); }
        $branches=BDH_Core::branches($repo);
        ...
        set_transient(self::init_key(),['fingerprint'=>$fingerprint,'repo'=>$repo,'branch'=>$branch,'manifestHash'=>$manifest['hash'],'blocked'=>$blocked],10*MINUTE_IN_SECONDS);
        return ['action'=>'initialize','repo'=>$repo,'branch'=>$branch,'syncState'=>'github_api_mode','expectedAction'=>$blocked?'blocked':'prepare_api_baseline','localAhead'=>0,'remoteAhead'=>0,'dirty'=>false,'files'=>$manifest['files'],'fileCount'=>$manifest['fileCount'],'sizeBytes'=>$manifest['sizeBytes'],'truncated'=>$manifest['truncated'],'reviewComplete'=>true,'blocked'=>$blocked,'blockReason'=>$reason,'fingerprint'=>$fingerprint];
    }

    public static function init_execute(string $fingerprint): array {
        $preview=get_transient(self::init_key());
        if (!is_array($preview) || !hash_equals((string)($preview['fingerprint']??''),$fingerprint)) { throw new RuntimeException('Initialization preview expired or changed. Review again.'); }
        if (!empty($preview['blocked'])) { throw new RuntimeException('This reviewed initialization is blocked.'); }
        $manifest=self::manifest();
        if (!hash_equals((string)$preview['manifestHash'],$manifest['hash'])) { throw new RuntimeException('Managed files changed after preview. Review again.'); }
        self::acquire('api-prepare');
        try {
            BDH_Core::save_selection((string)$preview['repo'],(string)$preview['branch']);
            BDH_Core::save(['github_api_mode'=>true,'github_api_prepared_at'=>gmdate('c'),'github_api_prepared_manifest'=>$manifest['hash']]);
            delete_transient(self::init_key());
            return ['ok'=>true,'logs'=>['GitHub API baseline prepared. No remote push was executed.'],'commit'=>'API','branch'=>(string)$preview['branch'],'repo'=>(string)$preview['repo'],'remotePushed'=>false];
    ...
    private static function changed_files(array $before, array $after): array {
        ...
        foreach ($before as $path=>$meta) { if (!isset($after[$path])) { $out[]=['direction'=>'local','status'=>'D','path'=>$path]; } }
        return $out;
    }
    ...
    public static function sync_preview(string $action): array {
        ...
        if ($action!=='push') {
            return ['action'=>$action,'repo'=>(string)(BDH_Core::state()['github_repo']??''),'branch'=>(string)(BDH_Core::state()['github_branch']??'main'),'syncState'=>'github_api_mode','expectedAction'=>'blocked','localAhead'=>0,'remoteAhead'=>0,'dirty'=>false,'files'=>[],'fileCount'=>0,'truncated'=>false,'reviewComplete'=>true,'blocked'=>true,'blockReason'=>'Hostinger API mode currently supports reviewed Push only.','fingerprint'=>''];
        }
        ...
        if (empty($state['github_api_prepared_at']) && empty($state['github_last_sync_commit'])) { throw new RuntimeException('Review First Push and prepare the API baseline first.'); }
        $manifest=self::manifest();
        $remoteHead=self::remote_head($repo,$branch);
        $lastRemote=(string)($state['github_last_sync_commit']??'');
        if ($remoteHead!=='' && $lastRemote==='' ) {
            return ['action'=>'push','repo'=>$repo,'branch'=>$branch,'syncState'=>'remote_changes','expectedAction'=>'blocked','localAhead'=>0,'remoteAhead'=>1,'dirty'=>false,'files'=>[],'fileCount'=>0,'truncated'=>false,'reviewComplete'=>true,'blocked'=>true,'blockReason'=>'Remote branch already exists but has no trusted Developer Hub baseline.','fingerprint'=>''];
        }
        if ($remoteHead!=='' && $lastRemote!=='' && !hash_equals($lastRemote,$remoteHead)) {
            return ['action'=>'push','repo'=>$repo,'branch'=>$branch,'syncState'=>'remote_changes','expectedAction'=>'blocked','localAhead'=>0,'remoteAhead'=>1,'dirty'=>false,'files'=>[],'fileCount'=>0,'truncated'=>false,'reviewComplete'=>true,'blocked'=>true,'blockReason'=>'Remote changed since the last Developer Hub push. Review remote changes before pushing.','fingerprint'=>''];
        }
        ...
        set_transient(self::push_key(),['fingerprint'=>$fingerprint,'repo'=>$repo,'branch'=>$branch,'remoteHead'=>$remoteHead,'manifestHash'=>$manifest['hash'],'expected'=>$expected],10*MINUTE_IN_SECONDS);
        return ['action'=>'push','repo'=>$repo,'branch'=>$branch,'syncState'=>$initial?'first_remote_push':'local_changes','expectedAction'=>$expected,'localAhead'=>$files?1:0,'remoteAhead'=>0,'dirty'=>!empty($files),'files'=>array_slice($files,0,500),'fileCount'=>count($files),'truncated'=>count($files)>500,'reviewComplete'=>true,'blocked'=>false,'blockReason'=>'','fingerprint'=>$fingerprint];
    }
    ...
    public static function sync_execute(string $action,string $fingerprint,string $message=''): array {
        ...
        if (!is_array($preview) || !hash_equals((string)($preview['fingerprint']??''),$fingerprint)) { throw new RuntimeException('Push preview expired or changed. Review again.'); }
        $repo=self::validate_repo((string)$preview['repo']); $branch=self::validate_branch((string)$preview['branch']);
        $manifest=self::manifest();
        if (!hash_equals((string)$preview['manifestHash'],$manifest['hash'])) { throw new RuntimeException('Managed files changed after preview. Review Push again.'); }
        $remoteHead=self::remote_head($repo,$branch);
        if (!hash_equals((string)$preview['remoteHead'],$remoteHead)) { throw new RuntimeException('Remote changed after preview. Review Push again.'); }
        if (($preview['expected']??'')==='noop') { return ['ok'=>true,'logs'=>['Already synchronized.'],'commit'=>$remoteHead,'branch'=>$branch,'repo'=>$repo,'remotePushed'=>false]; }
        ...
        try {
            ...
            $tree=[];
            if ($remoteHead==='') {
                foreach ($manifest['map'] as $path=>$meta) { $tree[]=['path'=>$path,'mode'=>'100644','type'=>'blob','content'=>$meta['content']]; }
                $treeResult=self::request('POST',$repo,'/git/trees',['tree'=>$tree]);
                $parents=[];
            } else {
                $commit=self::request('GET',$repo,'/git/commits/'.$remoteHead);
                $baseTree=(string)($commit['tree']['sha']??'');
                foreach ($manifest['map'] as $path=>$meta) {
                    if (!isset($before[$path]) || ($before[$path]['sha256']??'')!==$meta['sha256']) { $tree[]=['path'=>$path,'mode'=>'100644','type'=>'blob','content'=>$meta['content']]; }
                }
                foreach ($before as $path=>$meta) { if (!isset($manifest['map'][$path])) { $tree[]=['path'=>$path,'mode'=>'100644','type'=>'blob','sha'=>null]; } }
                if (!$tree) { return ['ok'=>true,'logs'=>['Already synchronized.'],'commit'=>$remoteHead,'branch'=>$branch,'repo'=>$repo,'remotePushed'=>false]; }
                $treeResult=self::request('POST',$repo,'/git/trees',['base_tree'=>$baseTree,'tree'=>$tree]);
                $parents=[$remoteHead];
            }
            $safe=trim(wp_strip_all_tags($message));
            if ($safe==='') { $safe=$remoteHead===''?'[Brando Developer Hub] Initial WordPress baseline':'Brando Developer Hub sync '.gmdate('Y-m-d H:i:s').' UTC'; }
            if (strlen($safe)>160) { $safe=substr($safe,0,160); }
            $newCommit=self::request('POST',$repo,'/git/commits',['message'=>$safe,'tree'=>(string)$treeResult['sha'],'parents'=>$parents]);
            $sha=(string)($newCommit['sha']??'');
            if ($sha==='') { throw new RuntimeException('GitHub did not return a commit SHA.'); }
            if ($remoteHead==='') { self::request('POST',$repo,'/git/refs',['ref'=>'refs/heads/'.$branch,'sha'=>$sha]); }
            else { self::request('PATCH',$repo,'/git/refs/heads/'.rawurlencode($branch),['sha'=>$sha,'force'=>false]); }
            $stored=[]; foreach ($manifest['map'] as $path=>$meta) { $stored[$path]=['sha256'=>$meta['sha256'],'size'=>$meta['size']]; }
            BDH_Core::save(['github_api_mode'=>true,'github_default_branch'=>$branch,'github_last_sync_at'=>gmdate('c'),'github_last_sync_commit'=>$sha,'github_last_sync_by'=>wp_get_current_user()->user_login,'github_api_last_map'=>$stored,'github_api_last_manifest'=>$manifest['hash']]);
            delete_transient(self::push_key());
            return ['ok'=>true,'logs'=>['GitHub API push completed.'],'commit'=>substr($sha,0,12),'branch'=>$branch,'repo'=>$repo,'remotePushed'=>true];
        } finally { self::release(); }
    }
}
