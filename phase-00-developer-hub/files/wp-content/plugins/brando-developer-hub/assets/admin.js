(() => {
  ...
  const notice = (message, type = 'info') => { const el=$('bdh-notice'); el.hidden=false; el.className=`bdh-notice bdh-${type}`; el.textContent=message; };
  const clearNotice = () => { $('bdh-notice').hidden = true; };
  const setBusy = (value) => { busy=value; app.classList.toggle('bdh-busy',value); app.querySelectorAll('button').forEach((button)=>{ button.disabled=value; }); };

  async function api(path, options = {}) {
    const response = await fetch(`${BDH.rest}${path}`, { credentials:'same-origin', ...options, headers:{'Content-Type':'application/json','X-WP-Nonce':BDH.nonce,...(options.headers||{})} });
  ...
  async function loadStatus() {
    ...
    try {
      ...
      setText('bdh-path',status.repoPath);
      setText('bdh-local-branch',status.branch||status.githubBranch||'—');
      setText('bdh-sha',status.shortSha||((status.githubLastSyncCommit||'').slice(0,12))||'—');
      setText('bdh-dirty',status.isDirty?t('نعم','Yes'):t('لا','No'));
      setText('bdh-unpushed',String(status.unpushedCount??0));
      setText('bdh-last-sync',fmtDate(status.githubLastSyncAt));
      const apiMode=status.executionMode==='github_api';
      const lastCommit=status.lastCommit || (apiMode ? t('وضع GitHub API — بدون Git CLI','GitHub API mode — no Git CLI') : t('لا يوجد Git repository في WordPress root','WordPress root is not a Git repository'));
      setText('bdh-last-commit',lastCommit);
      setText('bdh-connection',`${status.githubConnectionStatus||'disconnected'}${status.githubTokenSource?` • ${status.githubTokenSource}`:''}${apiMode?' • GitHub API mode':''}`);
      ...
      if(firstPushBox) firstPushBox.hidden=!!status.gitAvailable || !!status.githubLastSyncCommit;
  ...
  async function loadBranches(repo,selected='') {
    if(!repo) return;
    try {
      const data=await api(`/github-branches?repo=${encodeURIComponent(repo)}`,{method:'GET'}); const select=$('bdh-branch'); select.replaceChildren(); const branches=data.branches||[];
      ...
      if(!branches.length) { const branch=selected||'main'; const option=document.createElement('option'); option.value=branch; option.textContent=`${branch} ${t('(مستودع فارغ)','(empty repository)')}`; option.selected=true; select.appendChild(option); }
  ...
  async function reviewFirstPush() {
    if(busy) return; const repo=$('bdh-repo').value; const branch=$('bdh-branch').value||'main'; if(!repo) return notice(t('اختر المستودع أولاً','Choose the repository first'),'warning');
    setBusy(true); clearNotice();
    try { const data=await api('/repository-init-preview',{method:'POST',body:JSON.stringify({repo,branch})}); renderPreview(data); const sizeMb=((data.sizeBytes||0)/1048576).toFixed(1); notice(data.blocked?(data.blockReason||t('المراجعة محظورة','Review blocked')):`${t('مراجعة الـbaseline جاهزة','Baseline preview ready')} • ${data.fileCount||0} files • ${sizeMb} MB`,data.blocked?'warning':'success'); }
    catch(error) { notice(error.message,'error'); }
    finally { setBusy(false); if(preview) renderPreview(preview); }
  }
  ...
  $('bdh-execute').addEventListener('click',async()=>{
    if(!preview||preview.blocked) return; const isInit=preview.action==='initialize';
    if(!confirm(isInit?t('تجهيز baseline المراجع؟ لن يتم أي Push إلى GitHub.','Prepare the reviewed baseline? No GitHub push will be performed.'):t(`تنفيذ ${preview.action} بعد المراجعة؟`,`Execute reviewed ${preview.action}?`))) return;
    setBusy(true); clearNotice();
    try {
      const endpoint=isInit?'/repository-init-execute':'/github-sync-execute';
      const body=isInit?{fingerprint:preview.fingerprint,message:$('bdh-commit-message').value.trim()}:{action:preview.action,fingerprint:preview.fingerprint,message:$('bdh-commit-message').value.trim()};
      const data=await api(endpoint,{method:'POST',body:JSON.stringify(body)}); const logs=$('bdh-logs'); logs.hidden=false; logs.textContent=(data.logs||[]).join('\n\n')||t('تمت العملية بنجاح','Operation completed successfully');
      notice(`${isInit?t('تم تجهيز الـbaseline — اضغط Push يدويًا عندما تكون جاهزًا','Baseline prepared — click Push manually when ready'):t('تم التنفيذ','Executed')} • ${data.commit||''}`,'success'); preview=null; await loadStatus();
